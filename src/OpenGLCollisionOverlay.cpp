...
OpenGLCollisionOverlay::OpenGLCollisionOverlay(const CollisionGeometry& geometry)
    : m_LineRanges(geometry.lineRanges), m_VertexCount(geometry.vertices.size()),
      m_Bounds(geometry.bounds)
{
  ...
  f->glVertexAttribPointer(AttribPosition, 3, GL_FLOAT, GL_FALSE,
...
void OpenGLCollisionOverlay::render(QOpenGLShaderProgram* program,
                                    ...
                                    const QMatrix4x4& projectionMatrix) const
{
  ...
  for (const auto& range : m_LineRanges) {
    f->glDrawArrays(GL_LINES, static_cast<GLint>(range.firstVertex),
                    static_cast<GLsizei>(range.vertexCount));
  }
