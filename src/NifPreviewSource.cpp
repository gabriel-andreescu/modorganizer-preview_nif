...
QString relativeToBase(const QString& basePath, const QString& fileName)
{
  ...
  const auto relativePath =
      QDir(QDir::fromNativeSeparators(basePath)).relativeFilePath(fileName);
...
QString virtualPathFor(MOBase::IOrganizer* organizer, const QString& fileName)
{
  ...
  if (organizer) {
    ...
    if (const auto relativePath =
            relativeToBase(organizer->overwritePath(), normalized);
        !relativePath.isEmpty()) {
      return relativePath;
    }
  }
...
{
  return QFileInfo(lhs).absoluteFilePath().compare(QFileInfo(rhs).absoluteFilePath(),
                                                   Qt::CaseInsensitive) == 0;
}

bool sameDataPath(const QString& lhs, const QString& rhs)
{
  return normalizeDataPath(lhs).compare(normalizeDataPath(rhs), Qt::CaseInsensitive) ==
         0;
}
...
bool hasProviderPath(const QVector<NifPreviewProvider>& providers, const QString& path)
{
  return std::ranges::any_of(providers, [&](const NifPreviewProvider& provider) {
    return provider.kind == NifPreviewProviderKind::LooseFile &&
           !provider.absolutePath.isEmpty() && samePath(provider.absolutePath, path);
  });
}

bool hasArchiveProvider(const QVector<NifPreviewProvider>& providers,
                        const QString& archivePath, const QString& virtualPath)
{
  return std::ranges::any_of(providers, [&](const NifPreviewProvider& provider) {
    return provider.kind == NifPreviewProviderKind::Archive &&
...
QByteArray extractArchiveFile(const QString& archivePath, const QString& virtualPath)
{
  ...
  try {
    ...
    qWarning("Failed to load BSA archive '%s': %s", qUtf8Printable(archivePath),
             exception.what());
    return {};
  }
  ...
  const auto loadPath = [&](const QString& path) -> QByteArray {
    try {
      ...
      if (!blob.data || blob.size == 0 ||
          blob.size > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
        if (blob.size > static_cast<uint32_t>(std::numeric_limits<int>::max())) {
          qWarning("Skipping oversized NIF '%s' from BSA '%s'",
                   qUtf8Printable(virtualPath), qUtf8Printable(archivePath));
        }
        return {};
      }
...
void addArchiveProvider(QVector<NifPreviewProvider>& providers,
                        const QString& sourceName, const QString& virtualPath,
                        const QString& archivePath)
{
  ...
  const auto absoluteArchivePath =
...
void addArchiveProvidersFromMod(QVector<NifPreviewProvider>& providers,
                                MOBase::IModInterface* mod, const QString& virtualPath)
{
  if (!mod) {
    return;
  }
...
void addGameArchiveProviders(QVector<NifPreviewProvider>& providers,
                             MOBase::IOrganizer* organizer, const QString& virtualPath)
{
  if (!organizer) {
    return;
  }
...
int providerIndexForPath(const QVector<NifPreviewProvider>& providers,
                         const QString& path)
{
  if (path.isEmpty()) {
    return -1;
  ...
  for (int i = 0; i < providers.size(); ++i) {
    if (!providers[i].absolutePath.isEmpty() &&
        samePath(providers[i].absolutePath, path)) {
      return i;
    }
  }
...
int providerIndexForArchiveData(const QVector<NifPreviewProvider>& providers,
...
void addInMemoryProvider(QVector<NifPreviewProvider>& providers,
                         const QString& displayName, const QString& virtualPath,
...
NifPreviewSourceSet NifPreviewSourceResolver::resolve(MOBase::IOrganizer* organizer,
                                                      ...
                                                      const QByteArray& fileData)
{
    ...
    if (auto *const modList = organizer->modList()) {
      ...
      for (const auto& modName : origins) {
        ...
        const auto absolutePath =
            QDir(mod->absolutePath()).filePath(sourceSet.virtualPath);
        addLooseProvider(sourceSet.providers, mod->name(), sourceSet.virtualPath,
                         absolutePath);
  ...
  if (!fileData.isEmpty()) {
    if (const auto currentIndex =
            providerIndexForArchiveData(sourceSet.providers, fileData);
        currentIndex >= 0) {
      sourceSet.currentIndex = currentIndex;
    } else {
      addInMemoryProvider(sourceSet.providers, QObject::tr("Current Archive Preview"),
                          sourceSet.virtualPath, normalizedFileName, fileData);
      sourceSet.currentIndex = 0;
    }
  }

  if (fileData.isEmpty() && QFileInfo::exists(normalizedFileName)) {
    addLooseProvider(sourceSet.providers, QFileInfo(normalizedFileName).fileName(),
                     sourceSet.virtualPath, normalizedFileName);
    if (const auto currentIndex =
            providerIndexForPath(sourceSet.providers, normalizedFileName);
        currentIndex >= 0) {
      sourceSet.currentIndex = currentIndex;
    }
  }

  if (sourceSet.currentIndex < 0 ||
      sourceSet.currentIndex >= sourceSet.providers.size()) {
    sourceSet.currentIndex = 0;
  }
...
std::shared_ptr<nifly::NifFile> loadNifProvider(const NifPreviewProvider& provider)
{
  ...
  if (provider.kind == NifPreviewProviderKind::InMemory ||
      provider.kind == NifPreviewProviderKind::Archive) {
    const auto data =
        provider.kind == NifPreviewProviderKind::Archive && provider.data.isEmpty()
            ? extractArchiveFile(provider.archivePath, provider.virtualPath)
            : provider.data;
    if (data.isEmpty()) {
      return nullptr;
    ...
    nifFile = std::make_shared<nifly::NifFile>(
        std::filesystem::path(provider.absolutePath.toStdWString()));
  }
...
QString makeNifStatsText(const nifly::NifFile* nifFile)
{
  ...
  return QObject::tr("Verts: %1 | Faces: %2 | Shapes: %3")
      .arg(verts)
      .arg(faces)
      .arg(shapes);
}
