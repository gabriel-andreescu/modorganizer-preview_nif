...
struct SLSF2
{
  ...
  enum BSLightingShaderFlags2 : std::uint32_t
  {
    ...
    // Use Soft Lighting Map in non-PBR. Fuzz model in PBR. Parallax between layers in Two Layer model in PBR.
    SoftLighting = 1U << 25,
    // Use Rim Lighting Map in non-PBR. Use Subsurface shading in PBR.
    RimLighting = 1U << 26,
    // Use Back Lighting Map in non-PBR. Use Marschner hair shading model in PBR. Use coat normal map Two Layer model in PBR.
    BackLighting = 1U << 27,
    Unused02 = 1U << 28,
    TreeAnim = 1U << 29,
