...
nifly::MatTransform transformFromMatrix(const nifly::Matrix4& matrix,
                                        const float translationScale = 1.0f)
{
  nifly::MatTransform transform;
  transform.translation =
      nifly::Vector3(matrix[3], matrix[7], matrix[11]) * translationScale;
  ...
  transform.rotation = nifly::Matrix3(row0 / transform.scale, row1 / transform.scale,
                                      row2 / transform.scale);
  return transform;
}
...
{
  return std::fabs(lhs.r - rhs.r) <= Epsilon && std::fabs(lhs.g - rhs.g) <= Epsilon &&
         std::fabs(lhs.b - rhs.b) <= Epsilon && std::fabs(lhs.a - rhs.a) <= Epsilon;
}
...
public:
  explicit Builder(const nifly::NifFile* nifFile)
      : m_NifFile(nifFile), m_Header(nifFile->GetHeader()),
...
private:
  ...
  void addLegacyCollision(nifly::NiAVObject* object,
                          const nifly::MatTransform& transform)
  {
  ...
  void addBhkCollision(nifly::NiAVObject* object, const nifly::MatTransform& transform)
  {
    ...
    if (auto* rigidBodyT = dynamic_cast<nifly::bhkRigidBodyT*>(body)) {
      ...
      bodyTransform.translation =
          toVector3(rigidBodyT->translation) * BodyScale * m_HavokScale;
    }

    addBhkShape(m_Header.GetBlock<nifly::bhkShape>(body->shapeRef),
                transform.ComposeTransforms(bodyTransform), bodyLayerColor(body));
  }
  ...
  void addMultiBound(nifly::NiAVObject* object, const nifly::MatTransform& transform)
  {
    ...
    auto* multiBound =
        m_Header.GetBlock<nifly::BSMultiBound>(multiBoundNode->multiBoundRef);
    if (!multiBound) {
      return;
    }
    ...
    if (auto* aabb = dynamic_cast<nifly::BSMultiBoundAABB*>(data)) {
      addBox(transform, aabb->center - aabb->halfExtent,
             aabb->center + aabb->halfExtent, MultiBoundColor);
      ...
      addBox(transform.ComposeTransforms(obbTransform), obb->size * -1.0f, obb->size,
             MultiBoundColor);
  ...
  void addBhkShape(nifly::bhkShape* shape, const nifly::MatTransform& transform,
                   const CollisionColor& color)
  {
    if (!shape) {
      return;
    ...
    if (auto* list = dynamic_cast<nifly::bhkListShape*>(shape)) {
      ...
      addBhkShape(m_Header.GetBlock<nifly::bhkShape>(transformed->shapeRef),
                  transform.ComposeTransforms(localTransform), color);
      ...
      for (const auto& sphere : multiSphere->spheres) {
        addSphere(transform, sphere.center * m_HavokScale, sphere.radius * m_HavokScale,
                  color);
      }
      ...
      addCapsule(transform, capsule->point1 * m_HavokScale,
                 capsule->point2 * m_HavokScale,
                 std::max(capsule->radius1, capsule->radius2) * m_HavokScale, color);
    } else if (auto* strips = dynamic_cast<nifly::bhkNiTriStripsShape*>(shape)) {
      addNiTriStripsShape(
          strips, transform.ComposeTransforms(scaleTransform(1.0f / BodyScale)), color);
    ...
    } else if (auto* packedData =
                   dynamic_cast<nifly::hkPackedNiTriStripsData*>(shape)) {
  ...
  void addNiTriStripsShape(nifly::bhkNiTriStripsShape* shape,
                           const nifly::MatTransform& transform,
                           const CollisionColor& color)
  {
  ...
  void addPackedNiTriStripsShape(nifly::bhkPackedNiTriStripsShape* shape,
                                 const nifly::MatTransform& transform,
                                 const CollisionColor& color)
  {
    addPackedNiTriStripsData(
        m_Header.GetBlock<nifly::hkPackedNiTriStripsData>(shape->dataRef), transform,
        color);
  }

  void addPackedNiTriStripsData(nifly::hkPackedNiTriStripsData* data,
                                const nifly::MatTransform& transform,
                                const CollisionColor& color)
  {
    if (!data) {
  ...
  void addConvexVerticesShape(nifly::bhkConvexVerticesShape* shape,
                              const nifly::MatTransform& transform,
                              const CollisionColor& color)
  {
                              ...
                              const CollisionColor& color)
  {
    ...
    for (const auto& triangle : data->bigTris) {
      if (triangle.triangle1 < data->bigVerts.size() &&
          triangle.triangle2 < data->bigVerts.size() &&
          triangle.triangle3 < data->bigVerts.size()) {
        addTriangleEdges(
            transform, toVector3(data->bigVerts[triangle.triangle1]) * m_HavokScale,
            toVector3(data->bigVerts[triangle.triangle2]) * m_HavokScale,
            toVector3(data->bigVerts[triangle.triangle3]) * m_HavokScale, color);
      }
    }
  ...
  void addCompressedMeshChunk(nifly::bhkCompressedMeshShapeData* data,
                              ...
                              const CollisionColor& color)
  {
    ...
    for (std::size_t i = 0; i < vertices.size(); i++) {
      ...
      vertices[i] = transformRotation *
                    ((toVector3(chunk.translation) + transformTranslation + offset) *
                     m_HavokScale);
    }
    ...
    for (std::size_t i = offset; i + 2 < chunk.indices.size(); i += 3) {
      addTriangleEdges(vertices, chunk.indices[static_cast<std::uint32_t>(i)],
                       chunk.indices[static_cast<std::uint32_t>(i + 1)],
                       chunk.indices[static_cast<std::uint32_t>(i + 2)], transform,
                       color);
    }
  }

  void addBoundingVolume(const nifly::BoundingVolume& volume,
                         const nifly::MatTransform& transform,
                         const CollisionColor& color)
  {
    switch (volume.collisionType) {
    case nifly::SPHERE_BV:
      addSphere(transform, volume.bvSphere.center, volume.bvSphere.radius, color);
      break;
    case nifly::BOX_BV:
      addOrientedBox(transform, volume.bvBox.center, volume.bvBox.axis1,
                     volume.bvBox.axis2, volume.bvBox.axis3, volume.bvBox.extent1,
                     volume.bvBox.extent2, volume.bvBox.extent3, color);
      break;
    case nifly::CAPSULE_BV: {
      const auto capsuleAxis =
          normalized(volume.bvCapsule.origin) * volume.bvCapsule.extent;
      addCapsule(transform, volume.bvCapsule.center - capsuleAxis,
                 volume.bvCapsule.center + capsuleAxis, volume.bvCapsule.radius, color);
      break;
    }
    case nifly::UNION_BV:
      if (volume.bvUnion) {
  ...
  void addBox(const nifly::MatTransform& transform, const nifly::Vector3& min,
              const nifly::Vector3& max, const CollisionColor& color)
  {
    addBoxEdges(transform,
                {{
                    {min.x, min.y, min.z},
                    {max.x, min.y, min.z},
                    {max.x, max.y, min.z},
  ...
  void addOrientedBox(const nifly::MatTransform& transform,
                      const nifly::Vector3& center, const nifly::Vector3& axis1,
                      const nifly::Vector3& axis2, const nifly::Vector3& axis3,
                      const float extent1, const float extent2, const float extent3,
                      const CollisionColor& color)
  {
  ...
  void addBoxEdges(const nifly::MatTransform& transform,
  ...
  void addSphere(const nifly::MatTransform& transform, const nifly::Vector3& center,
                 const float radius, const CollisionColor& color)
  {
    if (radius <= Epsilon) {
      return;
    }
  ...
  void addCapsule(const nifly::MatTransform& transform, const nifly::Vector3& point1,
                  const nifly::Vector3& point2, const float radius,
                  const CollisionColor& color)
  {
    if (radius <= Epsilon) {
  ...
  void addCircle(const nifly::MatTransform& transform, const nifly::Vector3& center,
                 const nifly::Vector3& axisA, const nifly::Vector3& axisB,
                 const CollisionColor& color)
  {
    for (int i = 0; i < CircleSegments; i++) {
      const auto angleA =
          2.0f * Pi * static_cast<float>(i) / static_cast<float>(CircleSegments);
      const auto angleB =
          2.0f * Pi * static_cast<float>(i + 1) / static_cast<float>(CircleSegments);
      addLine(transform, center + axisA * std::cos(angleA) + axisB * std::sin(angleA),
              center + axisA * std::cos(angleB) + axisB * std::sin(angleB), color);
    }
  }

  void addTriangleEdges(const std::vector<nifly::Vector3>& vertices,
  ...
  void addTriangleEdges(const std::vector<nifly::Vector3>& vertices,
                        const std::uint16_t i1, const std::uint16_t i2,
                        const std::uint16_t i3, const nifly::MatTransform& transform,
                        const CollisionColor& color)
  {
  ...
  void addTriangleEdges(const nifly::MatTransform& transform,
                        const nifly::Vector3& point1, const nifly::Vector3& point2,
                        const nifly::Vector3& point3, const CollisionColor& color)
  {
    addLine(transform, point1, point2, color);
  ...
  void addLine(const nifly::MatTransform& transform, const nifly::Vector3& start,
               const nifly::Vector3& end, const CollisionColor& color)
  {
  ...
  void addLineRange(const std::size_t firstVertex, const std::size_t vertexCount,
                    const CollisionColor& color)
  {
    if (!m_Geometry.lineRanges.empty()) {
      auto& range = m_Geometry.lineRanges.back();
      if (sameColor(range.color, color) &&
          range.firstVertex + range.vertexCount == firstVertex) {
