...
NifPreviewPane::NifPreviewPane(MOBase::IOrganizer* organizer, QWidget* parent)
    : QWidget(parent), m_Organizer(organizer), m_Camera(makePaneCamera())
{
                                  ...
                                  const int currentIndex)
{
  ...
  for (const auto& provider : m_Providers) {
    ...
    m_SourceCombo->setItemData(m_SourceCombo->count() - 1, provider.displayName,
                               Qt::ToolTipRole);
  }
...
void NifPreviewPane::setCamera(QSharedPointer<Camera> camera)
{
  ...
  m_CameraConnection =
      connect(m_Camera.get(), &Camera::cameraMoved, this, &NifPreviewPane::cameraMoved);
  if (m_NifWidget) {
...
void NifPreviewPane::loadCurrentProvider()
{
  ...
  try {
    ...
    if (!nifFile) {
      qWarning("Failed to load NIF preview provider '%s'",
